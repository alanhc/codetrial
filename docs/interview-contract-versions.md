...
| Bundle | Introduced |
|---|---|
| 14 | Test counts, deltas, and diagnostics in the evidence projection retain their unverified browser provenance in live, interim, and final assessment prompts. Test event counts are labeled browser-reported, and the shared assessment heading distinguishes those claims from server-derived metadata. Each prompt gets only the evidence lines the rest of it does not state: a watch prompt the tests, diagnostics, hints, phases and session state, and only the lines that changed since the last watch prompt of the same session, with no evidence heading when none did; the interim review adds the code history and the time since the last program change; the report keeps the code history, the test history and the session state. The line of turn counts and the line of what arrived since the last watch prompt are gone, and the watch prompts point at `read_editor` in one short clause. The watch prompts themselves shrink to an event label, the evidence, the code and one action line naming the interview flow, the rules the system instructions already state no longer repeated in each; a proactive review is armed only by a change that is not a rename alone, in code that parses. A requested `log_hint` returns the candidate's editor with its clue, so a hint needs no `read_editor` first, and the report names its fields in a sentence where it printed a JSON example the response schema already enforces. The platform records the unasked STAR steps as skipped itself, at the five-minute warning and at the end, and the evidence tool no longer offers `skipped` or `session_timing`; code evidence needs no `read_editor` first, and the evidence reply names the phase instead of echoing the row. Watch prompts, test reactions, `read_editor`, the hint reply and the cold briefing share one record of the code the model was last shown, so a prompt with no change says the editor is unchanged; `read_editor` and the hint reply fence the editor and the latest run separately, with the timer last. Line numbers are unpadded, trailing blank lines are dropped, a whole buffer is never cut, and the whole-editor carriers stop at 32,000 bytes, with `read_editor` taking a `fromLine` to page past the cut its answer names. A test reaction lists one failing case, and every test account counts the failures it does not list from the reported totals, since the browser lists four at most. The live instructions state the hint and disclosure rules once, drop the constant per-question directions and the empty profile and grounding sections, and leave document grounding out of a coding-only session. The report and interim prompts send their constant rules as the system instruction and the interview material as the request; the report brief carries the contract and constraints the tests grade instead of the published statement, the interim note names the scenario, and the report schema no longer asks for the phase rows' weakness tags, which the server derives. |
