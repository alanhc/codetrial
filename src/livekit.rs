...
use crate::gemini::{
    GeminiEvent, GeminiLiveSession, generate_interim_review, open_live_session, redact_api_key,
    resume_live_session,
};
...
async fn handle_data_packet(
    ...
    // unasked steps, and the turns still open are the candidate's last words.
    // The report is written from this point while the farewell plays, so the
    // candidate waits for the longer of the two rather than for both, and the
    // farewell, which holds no evidence, stays out of what is assessed.
    close_turns(room, context).await?;
    ...
    let api_key = &interview.config.google_api_key;
    let farewell = async {
